...
/datum/gas_reaction/monkiumcondense //Condenses monkium into a monkey.
	priority = 100
	name = "Monkium condensation"
	id = "monkiumcondense"

/datum/gas_reaction/monkiumcondense/init_reqs()
	min_requirements = list(
		/datum/gas/monkium = 1000)

/datum/gas_reaction/monkiumcondense/react(datum/gas_mixture/air, datum/holder)
	var/old_heat_capacity = air.heat_capacity()
	var/monkies_created = round(air.get_moles(/datum/gas/monkium)/1000)
	var/energy_released = monkies_created * 4080

	if (isturf(holder))
		var/turf/open/location = holder
		air.adjust_moles(/datum/gas/monkium, -1000*monkies_created)
		for(var/i = 1 to monkies_created)
